...
/**
 * AuthPolicy Response from Custom Authorizer
 */
public class AuthPolicy {
    ...
    public static class PolicyDocument {
        ...
        static final String EXECUTE_INVOKE_API_ARN_FORMAT = "arn:aws:execute-api:%s:%s:%s/%s/%s";
        ...
        public void allowMethod(String resourcePath) {
            addResourceToStatement( allowStatement, resourcePath );
        }
        ...
        private void addResourceToStatement(Statement statement, String resourcePath) {
            // resourcePath must start with '/'
            // to specify the root resource only, resourcePath should be an empty string
            if (resourcePath.equals("/")) {
                resourcePath = "";
            ...
            String resource = resourcePath.startsWith("/") ? resourcePath.substring(1) : resourcePath;
            statement.addResource(String.format(EXECUTE_INVOKE_API_ARN_FORMAT, region, awsAccountId, restApiId, stage, resource));
        }
        ...
        public static PolicyDocument getAllowMultiplePolicy(String region, String awsAccountId, String restApiId, String stage, Set<String> resourcesPath) {
            AuthPolicy.PolicyDocument policyDocument = new AuthPolicy.PolicyDocument(region, awsAccountId, restApiId, stage);
            resourcesPath.stream().forEach( resource -> policyDocument.allowMethod( resource ) );
            return policyDocument;
        }
